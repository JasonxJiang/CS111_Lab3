...
add_block(ospfs_inode_t *oi)
{
	...
	{
		// if no indirect blocks then allocate
		if(oi->oi_indirect == 0)
		{
	...
	else if(n < OSPFS_MAXFILEBLKS)
	{
		...
		indirect = ((uint32_t *) ospfs_block(oi->oi_indirect2))[indir_index(n)];
		// if no second indirect block, allocate
		if (indirect == 0)
		{
		...
		memset(ospfs_block(direct), 0, OSPFS_BLKSIZE);
		((uint32_t *) ospfs_block(indirect))[direct_index(n)] = direct;
	}
	// n is greater than OSPFS_MAXFILEBLKS
	else
		return -ENOSPC;
...
remove_block(ospfs_inode_t *oi)
{
	...
	// free indirect, direct block
	else if(n < OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		uint32_t *indirect = (uint32_t*) ospfs_block(oi->oi_indirect);
		// free direct first
		free_block(indirect[direct_index(n)]);
		indirect[direct_index(n)] = 0;

		// if indirect block exists, free
		if(direct_index(n) == 0)
		{
	...
	else if(n < OSPFS_MAXFILEBLKS)
	{
		...
		uint32_t *indirect = (uint32_t *) ospfs_block(indirect2[indir_index(n)]);
		// free direct block first
		free_block(indirect[direct_index(n)]);
		indirect[direct_index(n)] = 0;
		...
		if(direct_index(n) == 0)
		{
			free_block(indirect2[indir_index(n)]);
			indirect2[indir_index(n)] = 0;
		}

		// if second indirect, free
		if(indir_index(n) == 0)
		{
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	// search for empty direntry
	uint32_t index;
	for(index = 0; index < dir_oi->oi_size; index += OSPFS_DIRENTRY_SIZE)
	{
		direntry = ospfs_inode_data(dir_oi, index);
		if(direntry->od_ino == 0)
			return direntry;
	}

	// if no empty direntry, add block
	int val = 0;
	val = (int)add_block(dir_oi);
	if(val < 0)
		return ERR_PTR(val);
	
	// add at index = current oi_size
	direntry = ospfs_inode_data(dir_oi, index);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	hardlink = create_blank_direntry(ospfs_inode(dir->i_ino));
	if(IS_ERR(hardlink))
	...
	//increase the source file link count
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	ospfs_direntry_t * direntry = NULL;
	...
	{
		inode = ospfs_inode(entry_inode);
		// if empty inode, break
		if(inode->oi_nlink == 0)
			break;
	}
	if (entry_inode == ospfs_super->os_ninodes)
		return -ENOSPC;
	...
	/* Execute this code after your function has successfully created the
	   ...
	   getting here. */
	{
		struct inode *i = ospfs_mk_linux_inode(dir->i_sb, entry_inode);
		if (!i)
			return -ENOMEM;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	// check if direntry exists
	if(find_direntry(ospfs_inode(dir->i_ino), dentry->d_name.name, dentry->d_name.len))
		return -EEXIST;

	// create new file with symlink
	entry_inode = ospfs_create(dir, dentry, dir_oi->oi_mode, NULL);
	if(entry_inode < 0)
		return entry_inode;

	entry_inode = find_direntry(ospfs_inode(dir->i_ino), dentry->d_name.name, dentry->d_name.len)->od_ino;
	symlink = (ospfs_symlink_inode_t*) ospfs_inode(entry_inode);
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// JIMIN
	// check for conditional symlink type
	if(strncmp(oi->oi_symlink, "root?", 5) == 0)
	{
		// find connection between first and second paths
		int conn = strchr(oi->oi_symlink, ':') - oi->oi_symlink;
		// user is root
		if(current->uid == 0)
		{
			oi->oi_symlink[conn] = '\0';
			// first path
		...
		{
			//second path
			nd_set_link(nd, oi->oi_symlink + conn + 1);
		}
	}
	else
